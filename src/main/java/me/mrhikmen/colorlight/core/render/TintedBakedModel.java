...
public class TintedBakedModel implements BlockStateModel, FabricBlockStateModel {
    ...
    // emitQuads переопределяет default-метод из FabricBlockStateModel (не из BlockStateModel!)
    @Override
    public void emitQuads(QuadEmitter emitter, BlockAndTintGetter blockView, BlockPos pos, BlockState state, RandomSource random, Predicate<Direction> cullTest) {
        ...
        emitter.pushTransform(quad -> {
            ...
            int a0 = ColorLightUtil.toArgb(c0, daylight);
            int a1 = (c1 == c0) ? a0 : ColorLightUtil.toArgb(c1, daylight);
            int a2 = (c2 == c0) ? a0 : (c2 == c1) ? a1 : ColorLightUtil.toArgb(c2, daylight);
            int a3 = (c3 == c0) ? a0 : (c3 == c1) ? a1 : (c3 == c2) ? a2 : ColorLightUtil.toArgb(c3, daylight);
        ...
        private void fillGrid(Direction face, int f) {
            ...
            for (int ia = 0; ia < 3; ia++) {
                for (int ib = 0; ib < 3; ib++) {
                    ...
                    opaque[idx] = view.getBlockState(cursor).getLightBlock() >= 15;
                }
            }
